...
function calculateRecruiterProfileCompletion(user: any, profile: any): number {
  const checks = [
...
router.get('/:userId', async (req: Request, res: Response) => {
  try {
    ...
    const profile = await (prisma as any).recruiterProfile.findUnique({
      where: { userId },
    })
...
router.put('/:userId', async (req: Request, res: Response) => {
  try {
    ...
    const profile = await (prisma as any).recruiterProfile.upsert({
      where: { userId },
      update: profileData,
      create: {
        userId,
