...
interface RecommendationResult extends Job {
  score: number
...
function requirementContains(requirement: string, keyword: string): boolean {
...
function calculateCandidateScore(
  ...
  const matchedSkills = candidateSkills.filter((skill) =>
    requirements.some((req) => requirementContains(req, skill))
  )

  const matchedInterests = candidateInterests.filter((interest) =>
    requirements.some((req) => requirementContains(req, interest))
  )
...
function calculateRecommendation(job: Job, userPreferences: any, userSkills: string[]): RecommendationResult {
  let score = 0
  const requirements = normalizeStringArray(job.requirements)
  const normalizedSkills = normalizeStringArray(userSkills)

  const mustHaves = normalizeStringArray(userPreferences?.mustHave)
  const matchedMustHave = mustHaves.filter((item) =>
    requirements.some((req) => requirementContains(req, item))
  )

  const niceToHave = normalizeStringArray(userPreferences?.niceToHave)
  const matchedNiceToHave = niceToHave.filter((item) =>
    requirements.some((req) => requirementContains(req, item))
  )
...
router.get('/recommend', async (req: Request, res: Response) => {
  try {
    ...
    const jobs = (await prisma.job.findMany({ where: { isActive: true } })) as Job[]

    const recommendedJobs = jobs
      .map((job) => calculateRecommendation(job, userPreferences, profile?.skills || []))
      .sort((a, b) => (b.score || 0) - (a.score || 0))
      .filter((job) => {
        const scorePass = (job.score || 0) >= minScore
        const locationPass =
          !locationFilter || String(job.location || '').toLowerCase().includes(locationFilter)
        return scorePass && locationPass
      })
      .slice(0, limit)
    ...
    if (recommendedJobs.length > 0) {
      await prisma.$transaction(
        recommendedJobs.map((job) =>
          (prisma as any).recommendationHistory.upsert({
            where: {
              userId_jobId: {
...
// Recommendation history (candidate)
router.get('/recommend/history/:userId', async (req: Request, res: Response) => {
  try {
    ...
    const history = await (prisma as any).recommendationHistory.findMany({
      where: { userId },
      include: {
        job: {
          select: {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!recruiter || (recruiter as any).userType !== 'RECRUITER') {
...
router.get('/recruiter/:userId', async (req: Request, res: Response) => {
  try {
    const userId = Number(req.params.userId)
...
router.get('/:id/recommended-candidates', async (req: Request, res: Response) => {
  try {
    const jobId = Number(req.params.id)
    ...
    const recommendedCandidates = users
      .filter((user) => (user as any).userType === 'CANDIDATE')
      .map((user) => {
...
router.post('/:id/apply', async (req: Request, res: Response) => {
  try {
    ...
    if (!user || (user as any).userType !== 'CANDIDATE') {
...
router.get('/applications/recruiter/:userId', async (req: Request, res: Response) => {
  try {
    const userId = Number(req.params.userId)
